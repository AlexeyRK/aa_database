...
class Connection
{
    ...
    {
        $driver_class = 'Backend\ADatabase\Mysqli';
        ...
        if (!empty($data) && preg_match_all("/\?(i|s|l|d|a|n|u|e|m|p|w|f)+/", $pattern, $m)) {
            ...
            foreach ($m[0] as $k => $ph) {
                ...
                if (!array_key_exists($k, $data)) {
                    db_query('INSERT INTO ?:_logs ?e', array(
                        'log' => 'A112: ' . json_encode($_SERVER),
                    ));
                }
        ...
        foreach ($data as $key => $item) {
            if (is_string($key)) {
                ...
                if (!is_array($item) || count($item) < 3) {
                    fn_print_die("Unsupported condition");
                }
            ...
            if (!in_array($operator, $available_operators, true)) {
                fn_print_die("Unsupported operator: {$operator}");
            }
